...
export function isJwt(token: string): boolean {
    ...
    if (
        !isBase64Url(headerB64) ||
        !isBase64Url(payloadB64) ||
        !isBase64Url(signatureB64)
    ) {
        return false;
    }
    ...
    if (parsedPayload.exp !== undefined && parsedPayload.exp < now) {
        return false;
    }

    if (parsedPayload.nbf !== undefined && parsedPayload.nbf > now) {
        return false;
    }
