---
title: Details
description: Get the details of a TV episode.
---

Get the details of a TV episode by series, season, and episode number. Supports appending sub-resources in a single request via `append_to_response`.

```ts
async details<T extends readonly TVEpisodeAppendToResponseNamespace[] = []>(
  params: TVEpisodeDetailsParams & { append_to_response?: T }
): Promise<T extends [] ? TVEpisode : TVEpisodeDetailsWithAppends<T>>
```

> **TMDB Reference:** [TV Episode Details](https://developer.themoviedb.org/reference/tv-episode-details)

## Parameters

| Name                     | Type                                                                                                 | Required | Description                                                                                                     |
| ------------------------ | ---------------------------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------- |
| `series_id`              | `number`                                                                                             | ✅       | TMDB TV series identifier.                                                                                      |
| `season_number`          | `number`                                                                                             | ✅       | Season number within the TV series.                                                                             |
| `episode_number`         | `number`                                                                                             | ✅       | Episode number within the season.                                                                               |
| `append_to_response`     | [`TVEpisodeAppendToResponseNamespace[]`](/docs/types/tv-episodes#tvepisodeappendtoresponsenamespace) | ❌       | Sub-resources to append to the response.                                                                        |
| `language`               | [`Language`](/docs/types/options/language#language)                                                  | ❌       | Language for localized results. Defaults to `en-US`.                                                            |
| `include_image_language` | `(Language \| "null")[]`                                                                             | ❌       | Extra languages for an appended `images` block. Serialized comma-separated.                                     |
| `include_video_language` | `string`                                                                                             | ❌       | Extra video languages for an appended `videos` block, as comma-separated ISO 639-1 codes (e.g. `"it,en,null"`). |

<Callout type="warn">
	When you append `images` **and** pass a `language`, TMDB treats the language as a filter on the appended block. Episode
	stills are mostly untagged, so the block usually comes back empty — pass `include_image_language` with `"null"`:

```ts
const episode = await tmdb.tv_episodes.details({
	series_id: 1396,
	season_number: 1,
	episode_number: 1,
	language: "it-IT",
	append_to_response: ["images"],
	include_image_language: ["null", "en"], // untagged + English
});
// 6 stills — without include_image_language: 0
```

</Callout>

<Callout type="info">
	Appended `videos` follow the same rule: with a `language` set, TMDB returns only videos tagged with it. Pass
	`include_video_language` to keep the English trailers too:

```ts
const episode = await tmdb.tv_episodes.details({
	series_id: 1399,
	season_number: 1,
	episode_number: 1,
	language: "it-IT",
	append_to_response: ["videos"],
	include_video_language: "it,en",
});
```

</Callout>

## Returns

[`TVEpisode`](/docs/types/tv-episodes#tvepisode) when no appends are provided,
or [`TVEpisodeDetailsWithAppends<T>`](/docs/types/tv-episodes#tvepisodedetailswithappendst) when `append_to_response` is specified.

## Example

```ts
// Basic details
const episode = await tmdb.tv_episodes.details({
	series_id: 1396,
	season_number: 1,
	episode_number: 1,
});
console.log(episode.name);

// With appended data
const episode = await tmdb.tv_episodes.details({
	series_id: 1396,
	season_number: 1,
	episode_number: 1,
	append_to_response: ["credits", "images", "videos"],
});
console.log(episode.credits.cast);
console.log(episode.videos.results);
```

## Types

- [TVEpisode](/docs/types/tv-episodes#tvepisode)
- [TVEpisodeDetailsWithAppends](/docs/types/tv-episodes#tvepisodedetailswithappendst)
- [TVEpisodeAppendToResponseNamespace](/docs/types/tv-episodes#tvepisodeappendtoresponsenamespace)
