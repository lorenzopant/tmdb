...
<Callout type="info">
	The cache is **in-process only**. It is not shared across server instances, workers, or requests
	in SSR environments. For persistent or shared caching, use the [`response.onSuccess`
	interceptor](/docs/getting-started/options/interceptors) to integrate with your own caching layer
	(Redis, LRU, etc.).
</Callout>
...
<Callout>
	...
	interceptor](/docs/getting-started/options/interceptors) to write responses to an external store
	(Redis, Memcached, etc.) and the [`request`
	interceptor](/docs/getting-started/options/interceptors) to read from it before the request is
	dispatched.
</Callout>
