...
<Callout>
	Deduplication is **on by default** and applied to every method in the client. Only concurrent
	requests benefit — sequential calls always produce independent fetches.
</Callout>
...
<Callout type="warn">
	Path fields such as `poster_path`, `backdrop_path`, `profile_path`, and `still_path` are optional
	on most TMDB response types. Always guard against `undefined` before building a URL.
</Callout>
...
<Callout>
	Rate limiting is **disabled by default**. Enable it when running bulk data-fetching scripts or
	background jobs that fire many requests in a tight loop — scenarios where hitting `429 Too Many
	Requests` would otherwise be likely.
</Callout>
...
<Callout>
	Response caching is **disabled by default**. The cache is in-process only — not shared across
	server instances or SSR requests. For persistent caching use the [`response.onSuccess`
	interceptor](/docs/getting-started/options/interceptors) to integrate with an external store.
</Callout>
